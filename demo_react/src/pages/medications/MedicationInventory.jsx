...
function MedicationInventory() {
  ...
  let filteredMedications = medications.filter(med => {
    ...
    const matchesExpiring = !showExpiring || 
                          (new Date(med.expiryDate).getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24) <= 90;
    
    return matchesSearch && matchesCategory && matchesRestock && matchesExpiring;
  });
  ...
  const getExpiryStatus = (expiryDate) => {
    ...
    if (daysUntilExpiry <= 0) {
      ...
      return { color: 'bg-yellow-100 text-yellow-800', text: 'Expires in 3 Months' };
    } else {
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="mb-8 flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600">Track and manage the school's medication supply</p>
        </div>
        <button 
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="flex items-center">
              ...
              />
              <label htmlFor="showRestock" className="ml-2 block text-sm text-gray-900">
                Needs Restock
              </label>
            </div>
            <div className="flex items-center">
              ...
              />
              <label htmlFor="showExpiring" className="ml-2 block text-sm text-gray-900">
                Expiring Soon
              </label>
            </div>
          </div>
          ...
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {filteredMedications.map((medication) => (
                <tr key={medication.id} className={medication.quantity <= medication.threshold ? 'bg-red-50' : ''}>
                  {editingId === medication.id ? (
                    ...
                    <>
                      ...
                      <td className="px-6 py-4">
                        <div className="text-sm text-gray-900">{medication.type}</div>
                      </td>
                      <td className="px-6 py-4">
                        <div className="text-sm text-gray-900">{medication.category}</div>
                      </td>
                      <td className="px-6 py-4">
                        <div className={`text-sm ${medication.quantity <= medication.threshold ? 'font-bold text-red-700' : 'text-gray-900'}`}>
                          {medication.quantity} / {medication.threshold}
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        <div className="text-sm text-gray-900">{formatDate(medication.expiryDate)}</div>
                        <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${getExpiryStatus(medication.expiryDate).color}`}>
                          {getExpiryStatus(medication.expiryDate).text}
                        </span>
                      </td>
                      <td className="px-6 py-4">
                        <div className="text-sm text-gray-900">{medication.storage}</div>
                      </td>
                      <td className="px-6 py-4">
                        <div className="text-sm text-gray-900">{formatDate(medication.lastRestocked)}</div>
                      </td>
                      <td className="px-6 py-4 text-right text-sm font-medium">
                        <button 
                          className="text-blue-600 hover:text-blue-900 mr-3"
                          onClick={() => {
                            const newQuantity = parseInt(prompt(`Current ${medication.name} stock: ${medication.quantity}\n\nEnter new quantity:`, medication.quantity));
        ...
        {filteredMedications.length === 0 && (
          <div className="text-center py-10">
            ...
            <p className="mt-1 text-sm text-gray-500">
              {searchTerm || filterCategory !== 'all' || showRestock || showExpiring
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-6">
        <div className="bg-white rounded-lg shadow p-6">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              ...
              <p className="text-3xl font-bold text-gray-700">{medications.length} items</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg shadow p-6">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              <h3 className="text-lg font-medium text-gray-900">Needs Restock</h3>
              <p className="text-3xl font-bold text-red-700">
                {medications.filter(med => med.quantity <= med.threshold).length} items
              </p>
            </div>
          </div>
        </div>
              ...
              <div className="grid grid-cols-2 gap-4 mb-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setNewMedication({...newMedication, threshold: parseInt(e.target.value) || 0})}
                  />
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4 mb-4">
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setNewMedication({...newMedication, storage: e.target.value})}
                  >
