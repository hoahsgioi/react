  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="mb-8">
        ...
        <p className="text-gray-600">View and respond to school health screenings</p>
      </div>
      ...
      {filteredCampaigns.length > 0 ? (
          ...
          {filteredCampaigns.map(campaign => (
            <div key={campaign.id} className="bg-white rounded-lg shadow overflow-hidden">
              ...
              <div className={`px-6 py-4 border-l-4 ${
                ...
                campaign.status === 'Upcoming' ? 'border-blue-500 bg-blue-50' :
              ...
              <div className="px-6 py-4">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div>
                    <h3 className="text-sm font-medium text-gray-500">Target Group</h3>
                    <p className="mt-1 text-sm text-gray-900">{campaign.targetGroup}</p>
                  </div>
                  <div>
                    <h3 className="text-sm font-medium text-gray-500">Health Checks</h3>
                    <p className="mt-1 text-sm text-gray-900">{campaign.screeningTypes.join(', ')}</p>
                  </div>
                  <div>
                    <h3 className="text-sm font-medium text-gray-500">Campaign Period</h3>
                    <p className="mt-1 text-sm text-gray-900">{formatDate(campaign.startDate)} - {formatDate(campaign.endDate)}</p>
                  </div>
                  <div>
                    <h3 className="text-sm font-medium text-gray-500">Location</h3>
                    <p className="mt-1 text-sm text-gray-900">{campaign.location}</p>
                  </div>
                  <div>
                    <h3 className="text-sm font-medium text-gray-500">Organized By</h3>
                    <p className="mt-1 text-sm text-gray-900">{campaign.organizer}</p>
                  </div>
                  <div>
                    <h3 className="text-sm font-medium text-gray-500">Consent Required</h3>
                    <p className="mt-1 text-sm text-gray-900">
                      {campaign.consentRequired ? (
                        <span>Yes (Deadline: {formatDate(campaign.consentDeadline)})</span>
                      ) : 'No'}
                    </p>
                  </div>
                </div>
                ...
                {(campaign.status === 'Active' || campaign.status === 'Completed') && (
                  <div className="mt-6">
                    <div className="flex justify-between items-center mb-1">
                      <h3 className="text-sm font-medium text-gray-700">Health Checks Completed</h3>
                      <span className="text-sm font-medium text-gray-700">
                        {campaign.checkupsCompleted} of {campaign.totalStudents} ({Math.round(campaign.checkupsCompleted/campaign.totalStudents*100)}%)
                      </span>
                    </div>
                    <div className="w-full bg-gray-200 rounded-full h-2.5">
                      <div 
                        className="bg-green-600 h-2.5 rounded-full" 
                        style={{ width: `${Math.round(campaign.checkupsCompleted/campaign.totalStudents*100)}%` }}
                      ></div>
                    </div>

                    <div className="flex justify-between items-center mt-4 mb-1">
                      <h3 className="text-sm font-medium text-gray-700">Consents Received</h3>
                      <span className="text-sm font-medium text-gray-700">
                        {campaign.consentsReceived} of {campaign.totalStudents} ({Math.round(campaign.consentsReceived/campaign.totalStudents*100)}%)
                      </span>
                    </div>
                    <div className="w-full bg-gray-200 rounded-full h-2.5">
                      <div 
                        className="bg-blue-600 h-2.5 rounded-full" 
                        style={{ width: `${Math.round(campaign.consentsReceived/campaign.totalStudents*100)}%` }}
                      ></div>
                    </div>
                  </div>
                )}
                ...
                <div className="mt-6 flex flex-col sm:flex-row sm:justify-end space-y-2 sm:space-y-0 sm:space-x-2">
                  ...
                  {(campaign.status === 'Active' || campaign.status === 'Completed') && campaign.checkupsCompleted > 0 && (
                    <Link 
        ...
        <div className="bg-white rounded-lg shadow p-8 text-center">
          ...
          <h3 className="mt-2 text-sm font-medium text-gray-900">No health check campaigns found</h3>
        ...
        <div className="flex">
          ...
          <div className="ml-3">
            <p className="text-sm text-blue-700">
              Regular health screenings help identify potential health issues early and ensure students' well-being. Please submit consent forms by the deadline to ensure your child can participate.
            </p>
          </div>
        </div>
      </div>
